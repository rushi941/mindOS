...
// Base prompt content (from prompt.txt, everything before "📝 REPORT OUTPUT REQUIREMENTS")
const BASE_PROMPT = `🧠 MINDSETOS STRATEGIC TEAM REPORT PROMPT — v11.1
...
export const generateReportWithAI = action({
  args: {
    team: v.any(),
    modules: v.array(v.any()),
  },
  handler: async (_ctx, args): Promise<{ markdown: string; version: string }> => {
    // Build prompt
    const prompt: string = await _ctx.runAction(api.promptBuilder.buildPrompt, {
      team: args.team,
    ...
    if (!process.env.OPENAI_API_KEY) {
      throw new Error("OPENAI_API_KEY not found in Convex environment variables");
    }
    ...
    return { markdown: text, version: "v1" };
  },
});
